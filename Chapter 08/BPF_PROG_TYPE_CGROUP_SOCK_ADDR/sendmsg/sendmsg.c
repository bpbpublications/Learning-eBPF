...
int main(int argc, char **argv) {
    ...
    obj = bpf_object__open_file("sendmsg.bpf.o", NULL);
    if (!obj) {
    ...
    struct bpf_program *prog = bpf_object__find_program_by_name(obj, "sendmsg_v4_prog");
    if (!prog) {
    ...
    if (bpf_prog_attach(prog_fd, cgroup_fd, BPF_CGROUP_UDP4_SENDMSG, 0)) {
